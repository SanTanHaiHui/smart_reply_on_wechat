...
import { callDoubaoStream } from "../../utils/doubao-api.js";
Component({
  properties: {
    chatMode: {
      type: String,
  ...
  attached: async function () {
    ...
    if (chatMode === "bot") {
      ...
      if (this.data.agentConfig.useDoubao) {
        // 使用豆包API时，不需要初始化bot，使用默认配置
        bot = {
          botId: "doubao",
          name: "接话",
          introduction: "帮你找到合适的回复",
          welcomeMessage: "我是聊天助手念念,不知道怎么回复,来问我吧",
  ...
  methods: {
    ...
    handleUploadImg: function (sourceType) {
      // 如果使用豆包API，不需要检查bot.searchFileEnable
      if (this.data.agentConfig.useDoubao) {
        // 豆包API支持图片上传，直接允许
      } else if (!this.data.bot.searchFileEnable) {
        wx.showModal({
          title: "提示",
    ...
    sendMessage: async function (message) {
      ...
      // 空消息返回（如果没有输入且没有文件）
      if (!inputValue && (!this.data.sendFileList || this.data.sendFileList.length === 0)) {
        return;
      }

      // 如果使用豆包API且有图片，确保图片状态为parsed
      if (this.data.agentConfig.useDoubao && this.data.sendFileList && this.data.sendFileList.length > 0) {
        const imageFiles = this.data.sendFileList.filter(item => item.rawType === 'image');
        if (imageFiles.length > 0) {
          // 确保所有图片都有tempPath且状态为parsed
          const invalidImages = imageFiles.filter(img => !img.tempPath || (img.status && img.status !== 'parsed'));
          if (invalidImages.length > 0) {
            // 自动更新状态
            const updatedFileList = this.data.sendFileList.map(item => {
              if (item.rawType === 'image' && (!item.status || item.status !== 'parsed')) {
                return {
                  ...item,
                  status: 'parsed'
                };
            ...
            this.setData({
              sendFileList: updatedFileList
            });
          }
        }
      }

      // 如果提供了prompt转换函数，则转换用户输入
      const promptTransform = this.data.promptTransform;
      if (promptTransform && typeof promptTransform === 'object' && promptTransform.transform) {
        try {
          // 传递文件列表信息，以便判断是否有图片
          const fileListForTransform = this.data.sendFileList || [];
          inputValue = promptTransform.transform(inputValue, fileListForTransform);
        } catch (e) {
          console.error('Prompt transform error:', e);
        }
      }

      const chatMode = this.data.chatMode;
      
      // 保存文件列表的副本，避免在setData后丢失
      const fileListCopy = [...(this.data.sendFileList || [])];
      const userRecord = {
        content: inputValue || "", // 如果没有输入文本，使用空字符串
        record_id: "record_id" + String(+new Date() - 10),
        role: "user",
        fileList: fileListCopy, // 使用副本
      };
      ...
      if (chatMode === "bot") {
        ...
        if (this.data.agentConfig.useDoubao) {
          // 使用豆包API
          const { doubaoApiKey, doubaoModel } = this.data.agentConfig;
          ...
          // 构建历史对话记录
          const history = [];
          const currentChatRecords = this.data.chatRecords;
          // 只取最近的对话记录（避免token过多）
          const recentRecords = currentChatRecords.slice(-10); // 最多10轮对话
          ...
          try {
            // 获取系统提示词（从父组件传递或使用默认值）
            let systemPrompt = null;
            if (this.data.agentConfig.systemPrompt) {
              systemPrompt = this.data.agentConfig.systemPrompt;
            } else if (this.data.promptTransform && typeof this.data.promptTransform.getSystemPrompt === 'function') {
              systemPrompt = this.data.promptTransform.getSystemPrompt();
            }
            
            // 获取图片列表（使用保存的副本）
            const images = fileListCopy || [];
            const imageList = images.filter(item => item.rawType === 'image');
            
            if (imageList.length > 0) {
              // 检查图片是否有有效的tempPath
              const validImages = imageList.filter(img => img.tempPath);
              
              if (validImages.length === 0) {
                wx.showModal({
                  title: "错误",
                  content: "图片路径无效，请重新选择图片",
                  showCancel: false,
                });
                this.setData({
                  chatStatus: 0,
              ...
              res = await callDoubaoStream({
                apiKey: doubaoApiKey,
                model: doubaoModel || "doubao-seed-1-6-flash-250828",
                message: inputValue,
                history: history,
              ...
              if (!isToolCallContent) {
                ...
                try {
                  ...
                  if (trimmedContent.startsWith('[') && trimmedContent.endsWith(']')) {
                    const parsedOptions = JSON.parse(trimmedContent);
                    if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
                      // 检查是否是回复选项格式（包含wording和reason）
                      const isValidFormat = parsedOptions.every(item => 
                        item && typeof item === 'object' && item.wording && item.reason
                      );
                      if (isValidFormat) {
                        lastValue.replyOptions = parsedOptions;
                        lastValue.content = ""; // 清空content，因为我们要用组件展示
                      }
                    }
                  } else {
                    // 尝试从content中提取JSON数组（可能content前后有其他文字）
                    // 使用贪婪匹配，匹配最长的[...]
                ...
                this.setData({
                  [`chatRecords[${lastValueIndex}].content`]: lastValue.content,
                  [`chatRecords[${lastValueIndex}].replyOptions`]: lastValue.replyOptions || null,
                  [`chatRecords[${lastValueIndex}].record_id`]: lastValue.record_id,
                  chatStatus: 3,
                }); // 聊天状态切换为输出content中
              }
        ...
        // 流式输出完成后，再次尝试解析JSON数组格式的回复选项
        if (!lastValue.replyOptions && lastValue.content) {
          try {
            const trimmedContent = lastValue.content.trim();
            
            // 方法1: 检查是否以[开头，以]结尾（纯JSON）
            if (trimmedContent.startsWith('[') && trimmedContent.endsWith(']')) {
              try {
                const parsedOptions = JSON.parse(trimmedContent);
                if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
            ...
            // 方法2: 如果方法1失败，尝试从content中提取JSON数组（可能content前后有其他文字）
            if (!lastValue.replyOptions) {
              // 尝试匹配最外层的JSON数组（使用贪婪匹配，匹配最长的[...]）
              const jsonMatch = trimmedContent.match(/\[[\s\S]*\]/);
              if (jsonMatch) {
                try {
                ...
                // 方法3: 尝试查找markdown代码块中的JSON
                const codeBlockMatch = trimmedContent.match(/```(?:json)?\s*(\[[\s\S]*?\])\s*```/);
                if (codeBlockMatch) {
                  try {
                    const parsedOptions = JSON.parse(codeBlockMatch[1]);
                    if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
                      const isValidFormat = parsedOptions.every(item => 
                        item && typeof item === 'object' && item.wording && item.reason
                      );
        ...
        if (lastValue.content === "" && !lastValue.replyOptions) {
          lastValue.content = this.data.defaultErrorMsg;
        }
        this.setData({
          [`chatRecords[${lastValueIndex}].content`]: lastValue.content,
        ...
        // 推荐问题功能（仅在使用腾讯云API时支持）
        if (bot.isNeedRecommend && !isManuallyPaused && !this.data.agentConfig.useDoubao) {
          const cloudInstance = await getCloudInstance(this.data.envShareConfig);
          const ai = cloudInstance.extend.AI;
